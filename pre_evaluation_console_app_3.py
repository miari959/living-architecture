...
# Add the specific experiment directory to path so we can import 'etl'
experiment_path = os.path.join(current_dir, 'applications', 'console_app_3')
...
# Import the Target Application (The ETL Pipeline)
try:
    from applications.console_app_3.etl import ETLPipeline
except ImportError:
    print("[CRITICAL] Could not import 'ETLPipeline'. Check your file structure.")
    sys.exit(1)


def run_evaluation():
    """
    Runs the 'ETL Pipeline' Console App through the evaluation configurations.
    For each configuration, it generates a JSON trace and a PlantUML diagram.

    NOTE: etl.py is single-threaded. The main challenge here is the loop in
    Transformer.clean_data() which iterates over 50 items — Stage 2 (depth
    limiting) and Stage 1b (utility filtering) are expected to reduce this noise.
    """
    ...
    print(f"[{'=' * 20} STARTING EVALUATION: CONSOLE APP 3 (ETL PIPELINE) {'=' * 20}]")
    ...
    for c in configs:
        ...
        json_path = f"output/console_app_3_{c['name']}.json"
        tracer.stop_trace(json_path)

        # 6. Generate PlantUML Diagram
        puml_path = f"output/console_app_3_{c['name']}.puml"
